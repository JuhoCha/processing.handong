...
void setup() {
  ...
  s1Xd=+1;
  s1Yd=+1; // 1번양 down right
  s1Xs=random(3, 6);
  s1Ys=s1Xs*0.1; // 양1 속도 10%
  s2X=width*3/4;
  s2Y=height*2/3;// 양2 위치
  s2Xd=-1;
  s2Yd=+1;// 2번양 down left
  s2Xs=random(2, 4);
  s2Ys=s2Xs*0.1;// 양2 속도 10%
...
void draw() {
  ...
  if (d < r + r && nFreeze ==0) { //두 양이 부딪히면 (물체감지 하면)
 ...
 println(eventStartTime);
  ...
  if (1000 < currentTime - eventStartTime && currentTime - eventStartTime < 2000 ) {//왼쪽 양
  } else {
    nFreeze =0;
