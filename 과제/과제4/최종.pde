...
void setup() {
  ...
  for (int i=0; i<100; i++) {
    sheepx[i] = random(0+10*sheepUnit, width-10*sheepUnit);
    sheepy[i] = random(0+10*sheepUnit, height-10*sheepUnit);
    sheepxd[i]=+1;
    sheepyd[i]=+1;
...
void draw() {
  ...
  for (int i=0; i<100; i++) {
    ...
    if (call && i == selected) {
      sheepx[selected] = sheepx[selected] + (textX - sheepx[selected])*0.02;
      sheepy[selected] = sheepy[selected] + (textY - sheepy[selected])*0.02;
    ...
    if (i == selected) {
      textSize(20);
      fill(#F20000);
      text(i, sheepx[i] - sheepUnit*5, sheepy[i] - abs(sin(jump_angle[i]))*maxH);
    } else {
      fill(0);
      textSize(10);
      text(i, sheepx[i] - sheepUnit*3, sheepy[i] - abs(sin(jump_angle[i]))*maxH);
    }
